...
public class HCluster {
     ...
     * @throws Exception
     */
    public Node[] hcluster(Instances data) throws Exception {
        ...
        //设置聚类簇数
        simpleKMeans.setNumClusters(data.numInstances() / 2);
        ...
        int nClusters = simpleKMeans.getNumClusters();

        //获得聚类索引
        Vector<Integer>[] nClusterID = new Vector[nClusters];
        for (int i = 0; i < nClusterID.length; i++) {
            nClusterID[i] = new Vector<>();
        }

        for (int i = 0; i < data.numInstances(); i++) {
            nClusterID[assignments[i]].add(i);
        }

        // used for keeping track of hierarchy
        Node[] clusterNodes = new Node[m_instances.numInstances()];

        getInstDistMatrix();
        for (int i = 0; i < nClusters; i++) {
            clusterNodes[i] = new Node(null, null, nClusterID[i]);
            clusterNodes[i].setVector(calcCentroId(nClusterID[i]));
            clusterNodes[i].setAvg(avg(distMatrix));
            clusterNodes[i].setEss(ess(clusterNodes[i].getAvg(),distMatrix));
            clusterNodes[i].setMax(calcMax(distMatrix));
            clusterNodes[i].setMin(calcMin(distMatrix));
        }
        ...
        m_nClusterNr = new int[m_instances.numInstances()];
        for (int i = 0; i < nClusterID.length; i++) {
     ...
     *
     * @param data 数据源
     * @param simpleKMeans  k-means 模型
     * @return
     * @throws Exception
     */
    public Node[] hcluster(Instances data,SimpleKMeans simpleKMeans ) throws Exception {
    ...
    void doLinkClustering(int nClusters, Vector<Integer>[] nClusterID, Node[] clusterNodes) {
        ...
        double[][] fDistance0 = distMatrix;

        //计算簇间距离
        for (int i1 = 0; i1 < nClusterID.length; i1++) {
            for (int i2 = i1 + 1; i2 < nClusterID.length; i2++) {
    ...
    protected  void  getInstDistMatrix() {
        int  nInstances =m_instances.numInstances();
        //fdistance0是单个实例距离矩阵
        ...
        for (int i = 0; i < nInstances; i++) {
            ...
            for (int j = i + 1; j < nInstances; j++) {
                fDistance0[i][j] = m_DistanceFunction.distance(m_instances.instance(i), m_instances.instance(j));
    ...
    void merge(int iMin1, int iMin2, double fDist1, double fDist2,
        ...
        Node node = new Node();
        //计算簇内距离
        double[][] metrics = metrics(distMatrix, nClusterID[iMin1]);
        ...
        //簇内样本的坐标
        node.setClusterInstIndex(nClusterID[iMin1]);
    ...
    public double[][] metrics(double[][] distMatrix, Vector<Integer> cluster1) {
    ...
    public Vector<Double> evaluate() {
        ...
        Vector<Integer>[] vectors = new Vector[m_clusters.length];
        for (int clst = 0; clst < vectors.length; clst++) {
            vectors[clst] = m_clusters[clst].getClusterInstIndex();
        }
